...
or updating a site never touches the distribution config.
The Router owns the distribution, so its
`domain` carries the certificate and DNS; an attached site's
`domain.name` is a host pattern matched at the edge.
