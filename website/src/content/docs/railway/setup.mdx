...
:::note[Docker and a registry for Effect-native Services]
[`Railway.Service`](/railway/compute/services) with `main` bundles
your Effect program, builds a `linux/amd64` image, and pushes it to
`registry` (GHCR or Docker Hub — Railway has no private registry of
its own). Set `registry` on the Service and one of:

- `RAILWAY_REGISTRY_USERNAME` + `RAILWAY_REGISTRY_PASSWORD`
- `GITHUB_ACTOR` + `GITHUB_TOKEN` (GHCR)
- `DOCKERHUB_USERNAME` + `DOCKERHUB_TOKEN`

Have [Docker](https://docs.docker.com/get-docker/) running on the
machine that runs `alchemy deploy`. Image-based Services
(`image: "hashicorp/http-echo"`) do not need Docker or a registry.
:::
