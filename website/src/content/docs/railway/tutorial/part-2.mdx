...
Railway container. Alchemy bundles it, builds a Docker image, and
pushes it to a registry Railway can pull. No Dockerfile.

:::note[Prerequisites]
Docker must be running. Alchemy builds `linux/amd64` images locally
and pushes them to `registry` (GHCR or Docker Hub). Set
`RAILWAY_REGISTRY` to a prefix you can push to (`ghcr.io/you`) and
one of `GITHUB_TOKEN`, `RAILWAY_REGISTRY_PASSWORD`, or
`DOCKERHUB_TOKEN`. Image-based Services
(`image: "hashicorp/http-echo"`) skip this — see
...
export default class Api extends Railway.Service<Api>()(
  "Api",
-  { project: Site, main: import.meta.url, registry: process.env.RAILWAY_REGISTRY ?? "ghcr.io/acme" },
+  { project: Site, main: import.meta.url, registry: process.env.RAILWAY_REGISTRY ?? "ghcr.io/acme", region: "us-west2" },
```
...
export default class Api extends Railway.Service<Api>()(
  "Api",
-  { project: Site, main: import.meta.url, registry: process.env.RAILWAY_REGISTRY ?? "ghcr.io/acme", region: "us-west2" },
+  { project: Site, main: import.meta.url, registry: process.env.RAILWAY_REGISTRY ?? "ghcr.io/acme", region: "us-west2", port: 3000 },
```
