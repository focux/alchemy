# @alchemy.run/web-frameworks/core
...
- **Framework packages** (`@alchemy.run/web-frameworks/waku`, `.../astro`,
...
- **Per-framework target modules** own the halves that are neither generic
  ...
  subpath of its framework package (e.g. `@alchemy.run/web-frameworks/waku/cloudflare`
  exporting the `WakuCloudflareTarget`). A future AWS target is a new subpath
  (`@alchemy.run/web-frameworks/waku/aws`) implementing the same seams — no change to
...
```ts
// packages/web-frameworks/src/waku/cloudflare.ts (illustrative)
...
1. Its options gain `target?: <Framework>TargetInput` (a
   ...
   cloudflare subpath specifier (e.g. `"@alchemy.run/web-frameworks/waku/cloudflare"`).
...
- **Waku** — `WakuTarget` extends `DeployTarget` with the adapter-path hook
  (today: the wrangler-free fork at `@alchemy.run/web-frameworks/waku/adapter`, selected
