# @alchemy.run/web-frameworks/astro
...
The package separates two concerns (see `packages/web-frameworks/src/core/README.md`,
...
| **Cloudflare target**            | `src/astro/cloudflare.ts` (subpath `@alchemy.run/web-frameworks/astro/cloudflare`), `src/astro/integration.ts`, `src/astro/config-plugin.ts`, `src/astro/prerenderer.ts`, `src/astro/prerender-environment.ts`, `src/astro/prerender-middleware.ts`, `src/astro/runtime/**` | The `AstroTarget` factory plus everything Cloudflare: the forked `@astrojs/cloudflare` integration over our vite plugin, the vendored runtime entrypoints, the `virtual:astro-cloudflare:config` plugin, the workerd prerenderer (build-side driver of the `__astro_*` protocol over cloudflare-runtime), the dev node-prerender middleware plugin. |
| **alchemy source provider**      | `src/astro/source.ts` (subpath `@alchemy.run/web-frameworks/astro/source`)                                                                                                                                                              | The alchemy `Cloudflare.Workers` source module (structural `SourceProvider` mirror): `build`/`hash`/`dev` for the alchemy Worker resource. Cloudflare-specific by definition; constructs the Cloudflare target directly.                                                                                                                            |
...
Omitting `target` defaults to `"@alchemy.run/web-frameworks/astro/cloudflare"`, so
...
| -------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `target`       | `AstroTargetInput`  | Deploy target (value / factory / specifier). Default: `"@alchemy.run/web-frameworks/astro/cloudflare"`.                                                                                                                                 |
...
export default Options.make({
  ...
  framework: "@alchemy.run/web-frameworks/astro",
});
...
- **Swapped plugin.** `@cloudflare/vite-plugin` → our plugin, `main` pinned to
  the vendored server entrypoint (`@alchemy.run/web-frameworks/astro/entrypoints/server`),
...
- **Kept (build-output parity).** The `astro:build:done` pass mirrors
  ...
  (`@alchemy.run/web-frameworks/astro/types.d.ts`).
...
- **`astro:env` at build time.** The alchemy source provider
  (`@alchemy.run/web-frameworks/astro/source`) feeds the Worker's literal env values
