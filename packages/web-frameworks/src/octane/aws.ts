/**
 * `@alchemy.run/web-frameworks/octane/aws` — the AWS Lambda deploy target
 * for the Octane integration.
 *
 * Octane's AWS story is its default (adapter-less) node server build:
 * with the marker adapter from
 * `@alchemy.run/web-frameworks/octane/aws-adapter` selected in
 ...
 *   `@alchemy.run/web-frameworks/octane/aws-adapter`; the framework half
...
export const ADAPTER_PACKAGE = "@alchemy.run/web-frameworks/octane/aws-adapter";
...
const resolveLambdaAdapterPath = Effect.try({
  try: () =>
    fileURLToPath(
      import.meta.resolve("@alchemy.run/web-frameworks/aws-lambda"),
    ),
  catch: (cause) =>
    fail(
      'Failed to resolve "@alchemy.run/web-frameworks/aws-lambda" — is the package built (its exports map serves dist/)?',
      cause,
    ),
});
