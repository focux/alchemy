/**
 * `@alchemy.run/web-frameworks/octane` — OctaneJS integration implementing
 * framework-core's `Framework` service, with the deploy target passed as a
 * value (Cloudflare Workers by default, via the
 * `@alchemy.run/web-frameworks/octane/cloudflare` subpath module).
 *
 ...
 * validation constants — lives behind `@alchemy.run/web-frameworks/octane/cloudflare`.
 */
...
/**
 * The e2e-harness factory contract (`framework: "@alchemy.run/web-frameworks/octane"`
 * in `e2e.config.ts`).
 */
const factory = (
