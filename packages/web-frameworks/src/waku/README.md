# @alchemy.run/web-frameworks/waku

Wrangler-free [Waku](https://waku.gg) integration. Implements
`@alchemy.run/web-frameworks/core`'s `Framework` service — programmatic
...
`packages/web-frameworks/src/core/README.md` → "Architecture: frameworks × deploy
...
| `./source` (`src/waku/source.ts`)         | Alchemy `Cloudflare.Worker` source provider (`{ provider: "@alchemy.run/web-frameworks/waku/source" }`) wrapping the framework with the cloudflare target | `cloudflare-runtime` (types)       |
...
### The cloudflare target (`@alchemy.run/web-frameworks/waku/cloudflare`)
...
```ts
import wakuFramework, { make } from "@alchemy.run/web-frameworks/waku";

make({
  /** Deploy target: value | (config) => target | module specifier |
   *  harness carriage. Default: "@alchemy.run/web-frameworks/waku/cloudflare",
...
- a **target value** — full type safety over the config; build it yourself:
  ...
  ```ts
  import cloudflareTarget from "@alchemy.run/web-frameworks/waku/cloudflare";
...
export default Options.make({
  framework: "@alchemy.run/web-frameworks/waku",
  target: {
    cloudflare: {
      worker: {
...
```ts
import wakuFramework from "@alchemy.run/web-frameworks/waku";
