/**
 * `@alchemy.run/web-frameworks/sveltekit` — SvelteKit integration implementing
 * framework-core's `Framework` service, with the deploy target passed as a
 * value (Cloudflare Workers by default, via the
 * `@alchemy.run/web-frameworks/sveltekit/cloudflare` subpath module).
 *
...
/**
 * The e2e-harness factory contract (`framework: "@alchemy.run/web-frameworks/sveltekit"`
 * in `e2e.config.ts`).
 */
const factory = (
  options: HarnessOptions,
